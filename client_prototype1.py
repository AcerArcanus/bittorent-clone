...
class peerComm:
  ...
  def peerReceive(incIP, incPort):
      sock.connect((incIP, incPort)) #connects to the peer its expecting a file from
      # receives file and prints when entire file is received
      file = b" "
      while True:
        fileChunk = sock.recv(1024)
        if not fileChunk:
    ...
    def sendPeerRequest(self, destIP, destPort):
        # connect to the peer that has the desired file
        sock.connect((destIP, destPort))
        # creates request for the specific file
        request = f"{self.fileName}|{self.srcIP}|{self.srcPort}"
        # send req and close connection
        sock.sendall(request.encode())
        sock.close()
        # run receive so the client is ready when the peer sends the file
    ...
    def receivePeerRequest(self):
        ...
        while True:
            ...
            #split request into desired fields
            requestedFile, destIP, destPort = request.split("|")
            #print confirms fields
            print("File requested:", requestedFile)
            print("Sending to:", destIP, destPort)
            # close the connection used for the request
            connection.close()
            # send the requested file to peer that made the request
            self.peerSend(destIP, int(destPort), requestedFile)
